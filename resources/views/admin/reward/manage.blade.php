                    ...
                    <div class="row align-items-center">
                        ...
                        <div class="col-sm-6">
                            <div class="float-right d-none d-md-block">
                                <div class="dropdown">
                                @if (Auth::guard('admin')->user()->can('reward.create'))
                                    <button class="btn btn-primary dropdown-toggle waves-effect waves-light" data-toggle="modal" data-target=".bs-example-modal-lg" type="button">
                                        <i class="far fa-calendar-plus  mr-2"></i> Add New Reward
                                    </button>
                                    @endif
                                </div>
                            </div>
...
Inactive
                                      ...
                                      @endif
                                                ...
                                                <td>
                                                    <div class="btn-group">
                                                    @if (Auth::guard('admin')->user()->can('reward.edit'))
                                                        <a type="button" href="{{ route('admin.reward.edit', $category->id) }}" class="btn btn-primary waves-light waves-effect"><i class="fas fa-pencil-alt"></i></a>
                                                        @endif
                                                        @if (Auth::guard('admin')->user()->can('reward.delete'))
                                                        <buttona type="button" class="btn btn-danger waves-light waves-effect" onclick="deleteTag({{ $category->id}})">
                                                        <i class="far fa-trash-alt"></i></button>
@endif
                                                        <form id="delete-form-{{ $category->id }}" action="{{ route('admin.reward.destroy',$category->id) }}" method="POST" style="display: none;">
                ...
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title mt-0" id="myLargeModalLabel">Add New Reward Point Name</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                        </div>
                        <div class="modal-body">
                            <form class="custom-validation" action="{{ route('admin.reward.store') }}" method="post">
                            @csrf
                                <div class="row">
                                    <div class="col-lg-12">
                                        <div class="card">
